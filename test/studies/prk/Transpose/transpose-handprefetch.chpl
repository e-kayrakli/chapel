...
for iteration in 0..iterations {
  ...
  if (tiled) {
    ...
    coforall l in Locales do on l {
      const opDom = A.localSubdomain();
      const opDomT = {opDom.dim(2), opDom.dim(1)};

      var localB = B[opDomT]; // create local copy

      forall (i,j) in B.localSubdomain() {
        localB[j,i] += A[i,j];
        A[i,j] += 1.0;
      }
      B[opDom.dim(2), opDom.dim(1)] = localB; // commit to global
    }
  }
} // end of main loop
