...
proc test(param dim:int, d: domain(dim)) {
  ...
  writeln("seeded").locale; showB.locale; showC.locale; showY.locale; showZ.locale;

  forall i in sd { A(i) = sumComps(i); }
  hd("forall(dom) { indexing = }").locale; showA.locale;

  forall x in X { x = 10; }
  hd("forall(arr) { iterator var = }").locale; showX.locale;

  forall (i,j) in zip(sd,sd) { A(i) += B(j); }
  hd("forall(dom,dom) { ix = ix }").locale; showA.locale;
  ...
  hd("forall(arr,arr) { ivar = ivar }").locale; showD.locale;

  forall (i,a) in zip(sd,A) { a = C(i); }
  hd("forall(dom,arr) { ivar = ix }").locale; showA.locale;

  forall (x,i) in zip(X,sd) { x = Z(i); }
  hd("forall(arr,dom) { ivar = ix }").locale; showX.locale;

  forall (i,j,k) in zip(sd,sd,sd) { Q(i) = Y(i) + Z(i); }
  hd("forall(dom,dom,dom) { ix = ix, ix }").locale; showQ.locale;

  forall (i,j,c) in zip(sd,sd,C) { A(i) = B(i) - c; }
  hd("forall(dom,dom,arr) { ix = ix, ivar }").locale; showA.locale;

  forall (i,z,x) in zip(sd,Z,X) { z = Y(i) - x; }
  hd("forall(dom,arr,arr) { ivar = ix, ivar }").locale; showZ.locale;

  forall (c,j,k) in zip(C,sd,sd) { A(j) = B(k) + c; }
  hd("forall(arr,dom,dom) { ix = ix, ivar }").locale; showA.locale;

  forall (z,x,k) in zip(Z,X,sd) { Q(k) = x + z; }
  hd("forall(arr,arr,dom) { ix = ivar, ivar }").locale; showQ.locale;

  forall (i,j,k,l) in zip(sd,sd,sd,sd) { D(l) = A(i) + B(j) + C(k); }
  hd("forall(dom,dom,dom,dom) { ix = 3*ix }").locale; showD.locale;

  forall (x,y,z,q) in zip(X,Y,Z,Q) { q = x + y + z; }
  hd("forall(arr,arr,arr,arr) { ivar = 3*ivar }").locale; showQ.locale;

  forall (i,j,a,b) in zip(sd,sd,A,B) { C(i) = D(j) - a - b; }
  hd("forall(dom,dom,arr,arr) { ix = ix, ivar, ivar }").locale; showC.locale;

  forall (x,q,i,j) in zip(X,Q,sd,sd) { q = Z(i) - x - Y(j); }
  hd("forall(arr,arr,dom,dom) { ivar = ix, ivar, ix }").locale; showQ.locale;

  forall (i,a,j,b) in zip(sd,A,sd,B) { D(i) = a - C(i) + b; }
  hd("forall(dom,arr,dom,arr) { ix = ivar, ix, ivar }").locale; showD.locale;

  forall(x,i,y,j) in zip(X,sd,Y,sd) { Q(i) = Z(i) - x - y; }
  hd("forall(arr,dom,arr,dom) { ix = ix, ivar, ivar }").locale; showQ.locale;

  A = B.locale; hd("A=B").locale; showA.locale;
  X = Y.locale; hd("X=Y").locale; showX.locale;
  D = A.locale; hd("D=A").locale; showD.locale;
  Q = X.locale; hd("Q=X").locale; showQ.locale;
  C = D.locale; hd("C=D").locale; showC.locale;
  B = C.locale; hd("B=C").locale; showB.locale;
  Z = Q.locale; hd("Z=Q").locale; showZ.locale;
  Y = Z.locale; hd("Y=Z").locale; showY.locale;
}
